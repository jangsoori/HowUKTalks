export default {
  birmingham: {
    coords: {
      lat: 52.49199,
      lng: -1.889045,
    },
    country: "gb",
    name: "Birmingham (Brummie)",
    id: "birmingham",
    videos: ["1K7xdiUv7So", "65xVky__r6U", "6v5A765sDUE"],
    words: [
      ["Bostin'", "Amazing / Excellent"],
      ["Our kid", "Younger Brother / Sister"],
      ["Babby", "Baby"],
      ["Bab", "Love / Dear"],
      ["Wench", "Girl / Young Woman"],
      ["Scrage", "Scratch (a skin)"],
      ["Fittle", "Food"],
      ["Yampy", "Mad / Losing the plot"],
      ["Piece", "Slice of bread and butter"],
      ["Never in a rain of pigs pudding", "Will never happen"],
      ["Noggy", "Outdated / Old-fashioned"],
      ["Cob", "Bread roll"],
      ["To Bawl", "To cry loudly"],
      ["Pop", "Fizzy drink"],
      ["Snap", "Food / Meal"],
    ],
  },
  bristol: {
    coords: {
      lat: 51.454597,
      lng: -2.587918,
    },
    country: "gb",
    name: "Bristol (Bristolian)",
    id: "bristol",
    videos: ["iXcizut9qA8", "59w6M7L2ijU", "731WC6dBMLU"],
    words: [
      ["Bemmie", "Bedminster"],
      ["Daps", "Plimsolls (shoes)"],
      ["Lamberts", "Cigarettes"],
      ["Drive", "Bus driver"],
      ["Drive", "Bus driver"],
      ["To scrage", "To scratch"],
      ["Gert lush", "Very nice"],
      ["Mua", "Mother"],
      ["Proper", "Good"],
      ["Innit", "Isn't it"],
      ["Brizzle", "Bristol"],
      ["Mint", "Really, really great"],
      ["Gashead", "Bristol Rovers Football Team supporter"],
      ["Keener", "Very eager to please"],
      ["Jason Donervan", "Best chip van in Bristol"],
    ],
  },
  cornwall: {
    coords: {
      lat: 50.264121,
      lng: -5.055756,
    },
    country: "gb",
    name: "Cornwall (Cornish)",
    id: "cornwall",
    videos: ["tcMJWZBzYjU", "Koxl4-t3i4M"],
    words: [
      ["Addled", "Empty / Broken"],
      ["Bal", "Mine"],
      ["Broder", "Brother"],
      ["Chacking", "Thirsty"],
      ["Dydhda", "Hello"],
      ["Ess", "Yes"],
      ["Fizzogg", "Face"],
      ["Giss on!", "Don't talk rubbish"],
      ["Henting", "Raining hard"],
      ["Hwoer", "Sister"],
      ["Jowse", "Shake / Rattle"],
      ["Mamm", "Mother"],
      ["Porth", "Harbour"],
      ["Skol", "School"],
      ["Squall", "To cry"],
    ],
  },
  edinburgh: {
    coords: {
      lat: 55.953253,
      lng: -3.188283,
    },
    country: "gb",
    name: "Edinburgh",
    id: "edinburgh",
    videos: ["JatocJtUQRM", "2lqS4JJ6Z9w"],
    words: [
      ["Aye", "Yeah"],
      ["Banter", "Good, witty chat"],
      ["Braw", "Nice / Brilliant"],
      ["Chancer", "A risker"],
      ["Chore", "To steal"],
      ["Dinnae", "Don't"],
      ["Fae", "From"],
      ["Fizzy juice", "Soda / soft drink"],
      ["Haud yer wheeshit", "Shut up"],
      ["Laddie/Lassie", "Boy/Girl"],
      ["Pie", "Ignore"],
      ["Radge", "Aggressive"],
      ["Rank", "Disgusting"],
      ["Skite", "Night out"],
      ["Shan", "Unfair"],
    ],
  },
  glasgow: {
    coords: {
      lat: 55.86422,
      lng: -4.251948,
    },
    country: "gb",
    name: "Glasgow (Glaswegian)",
    id: "glasgow",
    videos: ["extq6S6WZQU", "SGKoekcThLE", "QyOZTqTIF6s", "_FSWlfcg6oA"],
    words: [
      ["Scunnered", "Tired"],
      ["Dafty", "Stupid person"],
      ["Stoater", "Something fantastic"],
      ["Winch", "To kiss"],
      ["Piece", "Sandwich"],
      ["Gingie", "Bottle of soda"],
      ["Square go", "A fight"],
      ["Boak", "Disgusting"],
      ["Greet", "To cry"],
      ["Coupon", "Face"],
      ["Minted", "Rich"],
      ["Roaster", "Annoying person"],
      ["Dinghy", "To ignore someone"],
      ["Steamin'", "Drunk"],
      ["Haud yer wheesht", "Shut up"],
    ],
  },
  liverpool: {
    coords: {
      lat: 53.414913,
      lng: -2.991191,
    },
    country: "gb",
    name: "Liverpool (Scouse)",
    id: "liverpool",
    videos: ["hycOWmNWkHA", "mDtNE_QhO8A"],
    words: [
      ["Sound", "Cool / good"],
      ["Arl Fella", "Father"],
      ["Arl arse", "Cruel"],
      ["Wool", "Someone from surrounding areas of Liverpool"],
      ["G'wed", "Go ahead"],
      ["Lid", "Lad"],
      ["Boss", "Awesome"],
      ["Offie", "Off-Licence shop"],
      ["Antwacky", "Old fashioned"],
      ["Bifter", "Cigarettes"],
      ["Trabs", "Trainers (shoes)"],
      ["Scran", "Food"],
      ["Abar", "About"],
      ["The Ozzy", "Hospital"],
      ["Blert", "Idiot"],
    ],
  },
  london: {
    coords: {
      lat: 51.513902,
      lng: -0.09361,
    },
    country: "gb",
    name: "London (Cockney)",
    id: "london",
    videos: ["1WvIwkL8oLc", "LnjGNJ5JL8w"],
    words: [
      ["Adam and Eve", "Believe"],
      ["Apples and pears", "Stairs"],
      ["Ascot Races", "Idiot"],
      ["Aunt Joanna ", "Piano"],
      ["Baken Bean", "Queen"],
      ["Baker’s Dozen", "Cousin"],
      ["bird lime", "Time (in prison)"],
      ["Boat Race", "Face"],
      ["Bob Hope", "Soap"],
      ["Bottle and glass", "Arse"],
      ["Brass Tacks", "Facts"],
      ["Frog and Toad", "Road"],
      ["Roast Pork", "Fork"],
      ["Tea leaf", "Thief"],
      ["Wonga", "Cash"],
    ],
  },
  manchester: {
    coords: {
      lat: 53.481154,
      lng: -2.243506,
    },
    country: "gb",
    name: "Manchester (Mancunian)",
    id: "manchester",
    videos: ["GsrZk99s9LY", "eB2QbnyBe6s"],
    words: [
      ["Chuffed", "Pleased"],
      ["Hanging", "Disgusting"],
      ["Brew", "Cup of tea"],
      ["Sound", "Good"],
      ["Our kid", "Sibling"],
      ["Mint", "Very good"],
      ["Pop", "Fizzy drink"],
      ["Ginnel;", "Alleyway"],
      ["Dead", "Very"],
      ["Spitting", "Drizzling (rain)"],
      ["Cock", "Friend"],
      ["City", "Manchester City Football Club"],
      ["United", "Manchester United Football Club"],
      ["Chuddy", "Chewing gum"],
      ["Gaggin'", "Thirsty"],
    ],
  },
  newcastle: {
    coords: {
      lat: 54.983595,
      lng: -1.617461,
    },
    country: "gb",
    name: "Newcastle (Geordie)",
    id: "newcastle",
    videos: ["w2woc46LXkw"],
    words: [
      ["Aye", "Yes"],
      ["Nar / Nee", "No"],
      ["Wor", "Our"],
      ["Gan", "Going"],
      ["Yee", "You"],
      ["Neet", "Night"],
      ["Lang", "Long"],
      ["Howk", "Scratch"],
      ["Nowt", "Nothing"],
      ["Wrang", "Wrong"],
      ["Divvent", "Don't"],
      ["Alang", "Along"],
      ["Class", "Good"],
      ["Hinny", "Female"],
      ["Hoose", "House"],
    ],
  },
  norfolk: {
    coords: {
      lat: 52.626091,
      lng: 1.281731,
    },
    country: "gb",
    name: "Norfolk",
    id: "norfolk",
    videos: ["G0wrI-vqh54", "GVLiQy_-lXY"],
    words: [
      ["Afore", "Across"],
      ["Blar", "Cry"],
      ["Claggy", "Sticky"],
      ["Dawg", "Dog"],
      ["Fare", "To be"],
      ["Gret", "Great"],
      ["Haller", "Shout loudly"],
      ["Jip", "Pain "],
      ["Pamment", "Pavement"],
      ["Pishamire", "An ant"],
      ["Queer", "Ill / Sick"],
      ["Sadly", "Unwell"],
      ["Smur", "Drizzle"],
      ["Tidy", "Good"],
      ["Twizzle", "To spin"],
    ],
  },
  somerset: {
    coords: {
      lat: 51.015174,
      lng: -3.102662,
    },
    country: "gb",
    name: "Somerset",
    id: "somerset",
    videos: ["WnSrBNxNHVQ", "BErF_KpU5fw"],
    words: [
      ["Kiddie", "Man (younger than you, not a child)"],
      ["Lush", "Very nice"],
      ["Gert", "Great / Large"],
      ["Dimpsey", "Dusk"],
      ["Babber", "Baby / Child"],
      ["Betwaddled", "Confused"],
      ["Grockles", "Tourists"],
      ["Shag", "Mate"],
      ["Tent", "It isn't"],
      ["Yertiz", "Here it is"],
      ["Ark at 'Ee", "Listen to him"],
      ["Dreckly", "Soon"],
      ["Innit", "Isn't it"],
      ["Grass", "Snitch"],
      ["Off-key", "Someone having a problem / bad day"],
    ],
  },
  yorkshire: {
    coords: {
      lat: 53.963984,
      lng: -1.08833,
    },
    country: "gb",
    name: "Yorkshire",
    id: "yorkshire",
    videos: ["2Qx_2I9QBaA", "6jitXnalclc", "jzAD2GLfaNU"],
    words: [
      ["Innit", "Isn't it"],
      ["Arse", "An ass"],
      ["Back end", "Autumn"],
      ["Cake 'oil", "Mouth"],
      ["Dale", "Valley"],
      ["Doy", "Darling"],
      ["Eeh by gum", "Oh my god"],
      ["Flags", "Pavement"],
      ["Gaffer", "Boss (at work)"],
      ["Hacky", "Dirty / Sticky"],
      ["In a bit", "See you later"],
      ["Kegs", "Pants"],
      ["Monk on", "To be grumpy"],
      ["Over Yonder", "Over here"],
      ["Pack it in", "Stop it"],
    ],
  },
};
